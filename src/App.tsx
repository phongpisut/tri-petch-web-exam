...
function App() {
  ...
  const content = (
    ...
    <>
      <div
        className={`mb-5 col-span-1 tablet:col-span-2 desktop:col-span-1 pl-2 ${
          topic === "players" && "desktop:pl-36"
        }`}
      >
  ...
  const topics = (side: "left" | "right" = "left") => {
    ...
    return topic?.articles?.map((x, i) => (
      <MainGrid className={`${contentColor[i]}`} key={`${i}-article`}>
        {content(side === "left" ? "players" : "athlets", x, i, contentColor)}
      </MainGrid>
    ));
  };
